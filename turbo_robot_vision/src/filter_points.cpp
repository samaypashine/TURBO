/*
	...
	Edited by: Samay Pashine, Gyan Tatiya 
...
/*
	Function: seg_cb()
	Inputs  : turbo_robot_vision::TabletopPerception::Request &req, turbo_robot_vision::TabletopPerception::Response &res
	Outputs : bool
	Purpose : segments the cloud points and publishes the cloud points after filtering noise to rviz
*/
bool seg_cb(turbo_robot_vision::TabletopPerception::Request &req, turbo_robot_vision::TabletopPerception::Response &res)
{
...
{
	// Initialize ROS
	ros::init (argc, argv, "turbo_object_detector");
	ros::NodeHandle nh;
	...
	//debugging publisher
	cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("/turbo_object_detector/cloud", 10);

	//service
	ros::ServiceServer service = nh.advertiseService("/turbo_object_detector/detect", seg_cb); 
